...
static imengines_wp_plugin *wp;
static void *wp_handle;

int
imengines_wp_get_number_supported_languages(void)
{
...
imengines_im_status
imengines_wp_save_dictionary(unsigned int dictionary)
{
...
imengines_im_status
imengines_wp_add_word(const char *word, unsigned int dictionary,
...
imengines_im_status
imengines_wp_delete_word(const char *word, unsigned int dictionary,
                         unsigned int language_index)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_hit_word(const char *word, unsigned int dictionary,
                      unsigned int language_index)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
gboolean
imengines_wp_word_exists(const char *word, unsigned int dictionary,
                         unsigned int *language_index)
{
...
imengines_im_status
imengines_wp_get_max_candidates(int *number)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_set_max_candidates(int max_candidates)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_get_candidates(const gchar *previous_word,
...
imengines_im_status
imengines_wp_detach_dictionary(unsigned int dictionary)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_attach_dictionary(unsigned int dictionary, gboolean use_dictionary)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_set_prediction_language(const char *language,
                                     unsigned int language_index)
{
  g_return_val_if_fail(wp != NULL, IM_STATUS_FAILED);
...
imengines_im_status
imengines_wp_init(const char *wpengine)
{
  char str[1024];
  imengines_wp_plugin *(*func)();
  memset(str, 0, sizeof(str));
  /* FIXME - multiarch */
  snprintf(str, sizeof(str) - 1, "%s/libim_wp_plugin_%s.so",
           "/usr/lib/libimengines", wpengine);

  if (wp)
    imengines_wp_destroy();

  wp = NULL;
  wp_handle = dlopen(str, RTLD_NOW);

  if (wp_handle)
  {
    func = (imengines_wp_plugin *(*)())dlsym(wp_handle,
                                             "imengines_wp_plugin_init");
    ...
    if (func)
    {
      ...
      g_warning("Failed calling init function for WP");
    }
    else
      g_warning("Can't find init function in WP plugin %s", wpengine);
  }
  else
    g_warning( "Can't open WP plugin library %s", str);
